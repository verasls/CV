...
print_experience <- function(cv) {
  ...
  # Remove the institution and location if none is provided
  if (any(is.na(d$loc))) {
    na <- which(is.na(d$loc))
  }
  for (i in na) {
    to_sub <- stringr::str_locate(exp[[i]], "map-marker")[[1]] - 30
    substr(exp[[i]], to_sub, to_sub) <- "%"
  }
  if (any(is.na(d$institution))) {
    na <- which(is.na(d$institution))
  }
  for (i in na) {
    to_sub <- stringr::str_locate(exp[[i]], d$title[1])[[2]] + 2
    substr(exp[[i]], to_sub, to_sub) <- "%"
  }
  exp
}
